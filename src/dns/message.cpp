...
using namespace DNS;
std::vector<uint8_t> DNS::Message::encode_query(const std::string& name, RRType type, uint16_t id) {
...
void Message::encode_uint16(std::vector<uint8_t>& out, uint16_t v) {
    out.push_back(static_cast<uint8_t>(v >> 8));
    out.push_back(static_cast<uint8_t>(v & 0xFF));
}

void Message::encode_name(std::vector<uint8_t>& out, const std::string& name) {
...
void Message::encode_uint32(std::vector<uint8_t>& out, uint32_t v) {
...
std::optional<MessageHeader> Message::decode(std::span<const uint8_t> wire) {
    if (wire.size() < 12) {
        return std::nullopt;
    }
...
bool Message::decode_header(std::span<const uint8_t> wire, size_t& off, MessageHeader& msg) {
    if (wire.size() < 12) {
        return false;
    }
...
bool Message::decode_question(std::span<const uint8_t> wire, size_t& off, Question& q) {
...
bool Message::decode_rr(std::span<const uint8_t> wire, size_t& off, ResourceRecord& rr) {
...
bool Message::decode_name(std::span<const uint8_t> wire, size_t& off, std::string& out, int depth) {
...
bool Message::decode_rdata(std::span<const uint8_t> wire, size_t& rdata_off, uint16_t rdlength, RRType type, RData& out) {
    switch (type) {
    case RRType::A: {
        if (rdlength != 4) {
            return false;
...
uint16_t Message::read_u16(std::span<const uint8_t> wire, size_t off) {
    return static_cast<uint16_t>((wire[off] << 8) | wire[off + 1]);
}

uint32_t Message::read_u32(std::span<const uint8_t> wire, size_t off) {
