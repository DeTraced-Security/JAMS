...
Resolver::Resolver(const std::string& nameserver, io_uring* ring) : ring_(ring), rng_(std::random_device{}()) {
...
void Resolver::resolve(const std::string& name, RRType type, ResolveCallback cb) {
    ...
    pq.retries = 0;
    pq.wire = Message::encode_query(name, type, txid);
...
void Resolver::resolve_txt(const std::string& name, ResolveCallback cb) {
    resolve(name, RRType::TXT, std::move(cb));
}

void Resolver::resolve_mx(const std::string& name, ResolveCallback cb) {
    resolve(name, RRType::MX, std::move(cb));
}

void Resolver::resolve_a(const std::string& name, ResolveCallback cb) {
    resolve(name, RRType::A, std::move(cb));
}

void Resolver::on_cqe(uint64_t user_data, int res) {
...
void Resolver::submit_query(uint16_t txid, PendingQuery& pq) {
    // SendMsg SQE
    {
...
void Resolver::arm_recv(uint16_t txid) {
...
void Resolver::on_send(uint16_t txid, int res) {
    if (res < 0) {
...
void Resolver::on_recv(uint16_t txid, int res) {
    ...
    if (res == -ETIME || res == -ECANCELED) {
        if (pq.retries < MAX_RETRIES) {
            ...
            pq.wire = Message::encode_query(pq.name, pq.type, new_txid);
...
void Resolver::on_timeout(uint16_t txid) {
    logger.info("[DNS] Timeout SQE fired txid=" + std::to_string(txid));
}

uint16_t Resolver::alloc_txid() {
    uint16_t txid;
