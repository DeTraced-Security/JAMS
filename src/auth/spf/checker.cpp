...
Checker::Checker(DNS::Resolver& resolver) : resolver_(resolver) {};
...
void Checker::fetch_and_eval(const std::string& domain, std::shared_ptr<EvalState> state) {
...
void Checker::handle_txt_response(
...
bool Checker::parse_record(const std::string& txt, SPFRecord& out) {
...
bool Checker::parse_mechanism(const std::string& token, Mechanism& out) {
    size_t start = 0;
...
Result Checker::qualifier_to_result(char q) {
    switch (q) {
    case '+': {
        return Result::Pass;
    }
...
void Checker::evaluate(const SPFRecord& record, std::shared_ptr<EvalState> state) {
...
void Checker::eval_mechanisms_at(
...
bool Checker::ipv4_matches(const std::string& client_ip, const std::string& cidr) {
...
bool Checker::ipv6_matches(const std::string& client_ip, const std::string& cidr) {
    auto slash = cidr.find('/');
    std::string base_str = (slash == std::string::npos) ? cidr : cidr.substr(0, slash);
...
std::string Checker::extract_domain(const std::string& mail_from) {
...
uint8_t Checker::prefix_len(const std::string& cidr, uint8_t default_len) {
    auto slash = cidr.find('/');
...
void Checker::finish(std::shared_ptr<EvalState> state, Result result, std::string explanation) {
